                ...
                <div class="col-md-8 col-xl-9 middle-wrapper">
                  ...
                  <div class="card">
                    <div class="card-body">
                                      ...
                                      <h6 class="card-title">Update Role for user: <span style="color: aqua">{{$user->firstname}} {{$user->lastname}}</span></h6>
      
                                      <form method="POST" action="{{url('submit_role_update' , $user->id)}}">
                                        @csrf
                                        @method('PATCH')
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            <label for="role" class="form-label">Select Role to Update</label>
                                            <select class="form-control combo-box" id="role" name="role" required>
                                                <option value="" disabled selected>Select a role</option>
                                                @foreach($roles as $role)
                                                    <option value="{{ $role->name }}" {{ $user->getRoleNames()->contains($role->name) ? 'selected' : '' }}>
                                                        {{ $role->name }}
                                                    </option>
                                                @endforeach
                                            </select>
